...
/* Returns the amount of bytes has been read */
long int
avro_reader_file_ftell(avro_reader_t reader)
{
    if (is_file_io(reader))
    {
        struct _avro_reader_file_t *reader_file = avro_reader_to_file(reader);
        long int buffered_bytes = reader_file->cur != reader_file->end ? bytes_available(reader_file) : 0;

        return (ftell(reader_file->fp) - buffered_bytes);
    }
    else if (is_memory_io(reader))
    {
...
int avro_reader_file_fseek(avro_reader_t reader, long int new_position)
{
    ...
    struct _avro_reader_file_t *reader_file = avro_reader_to_file(reader);
    buffer_reset(reader_file);
    return fseek(reader_file->fp, new_position, SEEK_SET);
}
